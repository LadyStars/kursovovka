    ...
    new_db.IDcheck(USRlogIn);
        ...
        new_auth.CompareHashes(pass);
            ...
            for(unsigned int i =0; i< num_vectors; i++) {
                ...
                if (bytes_read != static_cast<int>(vector_len*sizeof(int16_t))){
                	throw server_error("Invalid type of date");
                }
                std::vector<int16_t> arr;
                for(unsigned int i =0; i< vector_len; i++) {
                    arr.push_back(int_buf[i]);
                }
                new_manager.sending(sock, Counter().summ(arr), sizeof(int16_t));
            }
            close(sock);
            std::cout<<"Done!\n";
            return 1;
